...
public class NewBeatScript : MonoBehaviour
{
    ...
    private Color decreasingOpacity = Color.white;

    //Script by Vacciones_Mad at https://answers.unity.com/questions/733587/beat-detection-algorithm.html
    //Code modified to spawn beats following the metronome
    ...
    void Update()
    {
        ...
        { // now we check if we have a beat
            if (!Beated)
            {
                currentBeat = Instantiate(beatObject, transform.position, transform.rotation);
                switch (ScoreManager.instance.ScoreMult)
                {
                    case 1:
                        decreasingOpacity.a = 1;
                        break;
                    case 2:
                        decreasingOpacity.a = .6f;
                        break;
                    case 3:
                        decreasingOpacity.a = .3f;
                        break;
                    case 4:
                        decreasingOpacity.a = 0;
                        break;
                ...
                currentBeat.GetComponent<SpriteRenderer>().color = decreasingOpacity;
                Debug.Log("Beat");
                Beated = true;
            }
        }
